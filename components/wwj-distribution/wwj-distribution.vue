			...
			<dz-grid :col="config.props.cell" :border="false" style="margin-top: -20rpx;" v-if="userPromoter">
				...
				<block v-if="config.props.mode == 'store'">
					<dz-grid-item v-if="config.props.store[0].checked">
						<view class="">{{ userPromoter.countExtend || '0' }}人</view>
						<view class="dz-font-sm dz-color-6">{{ config.props.store[0].title }}</view>
					</dz-grid-item>
					<dz-grid-item v-if="config.props.store[1].checked">
						<view class="dz-font-price">{{ userPromoter.sumUnWithdraw || '0.00' }}</view>
						<view class="dz-font-sm dz-color-6">{{ config.props.store[1].title }}</view>
					</dz-grid-item>
					<!-- <dz-grid-item v-if="config.props.store[1].checked" @click="frozenCommissionClick">
						<view class="dz-font-price">{{ userPromoter.sumUnWithdraw || '0.00' }}</view>
						<view class="dz-font-sm dz-color-6">
							{{ config.props.store[1].title }}
							<dz-icon style="margin-left:10rpx" name="questionfill" size="28"></dz-icon>
						</view>
					</dz-grid-item> -->
					<dz-grid-item v-if="config.props.store[2].checked">
						<view class="dz-font-price">{{ userPromoter.sumCommission || '0.00' }}</view>
						<view class="dz-font-sm dz-color-6">{{ config.props.store[2].title }}</view>
					</dz-grid-item>
				</block>
			</dz-grid>
			<dz-modal
