<template>
		<view
			...
			@tap.stop="$u.throttle(clickDoll(data), 500)"
			...
			<view class="dz-product-image-wrapper">
				...
				<!-- <image :style="{borderRadius:`${radius}rpx`}" :src="$api.helper.formatImg(data.thumbnail)" :mode="direction==='horizontal' ? 'aspectFill' : 'widthFix'" :preview="false" :class="[direction==='horizontal' ? 'dz-product-list-img' : 'dz-product-img']"></image> -->
			...
			<view class="dz-pro-content">
				<view class="dz-pro-tit">
					<!-- <dz-icon  class="dz-m-r-10" top="8rpx" :color="theme.dzBaseColor" :name="$api.assetsConfig.gamePlayingGif" :height="30" :width="30"></dz-icon> -->
					<!-- <dz-icon class="dz-m-r-10" :color="theme.dzBaseColor" name="idle" v-else></dz-icon> -->
					{{ data.name }}
				</view>
				<view class="line">
					<view class="dz-pro-price">
						<dz-tag  mode="plain" type="info" size="mini" shape="radius">
							<dz-icon :name="$api.assetsConfig.coin" :size="23"></dz-icon>
							<text class="dz-sale-price" :style="{ color: theme.dzMainColor }">
								{{ data.coin}}
							</text>
							<text class="dz-sale-price-unit" :style="{ color: theme.dzMainColor }">
								币
							</text>
						</dz-tag>
					...
					<view class="dz-pro-price">
						<dz-tag mode="plain" type="info" size="mini" shape="radius">
							<text style="color: #909399;">{{data.machineIds ? data.machineIds.length : 0}}台</text>
						</dz-tag>
					</view>
				</view>
			</view>
	...
	methods: {
		clickDoll(item) {
			...
			if(!machineIds || machineIds.length == 0){
				uni.showToast({title:"无可用机器~", icon:"none", mask:true});
				return;
