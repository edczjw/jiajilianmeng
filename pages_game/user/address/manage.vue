<template>
	<view>
		...
		<view class="content">
			...
			<view class="row-address">
				...
				<view class="dz-flex" style="height: 120rpx;">
					...
					<view class="dz-flex-9">
						<dz-input type="text" v-model="addressData.phone" :placeholder="language.address.addPhoneNumber" style="font-size: 28rpx;width: 100%;" />
					</view>
				</view>
			</view>
			<view class="row-address" @click="showPicker = !showPicker">
			...
			<dz-field
				class="row-address"
				v-model="addressData.addressDetails"
				:borderBottom="false"
				label-width="140"
				label-align="left"
				...
				:fieldStyle="{ width: '500rpx' }"
			></dz-field>
			<view class="row-address" style="margin-top: 20rpx;">
				<view class="nominal" style="width: 140rpx;font-size: 28rpx;">{{ language.address.setDefaultAddress }}</view>
				<view class="input switch">
					<dz-switch v-model="addressData.isDefault == 1 ? true : false" :active-color="theme.dzBaseColor" @change="isDefaultChange(addressData.isDefault)"></dz-switch>
				</view>
			</view>
			<view class="row-flex">
...
export default {
	data() {
		return {
			...
			addressData: {
				...
				phone: '',
				addressDetails: '',
				addressName: this.$api.language.address.pleaseChooseAddress,
				address: '',
				isDefault: false,
				province_id: '',
				city_id: '',
	...
	async onLoad(e) {
		...
		let addressData;
		if(e.address){
			addressData = JSON.parse(e.address);
		...
		if (this.editType == 'edit') {
			...
			this.address = this.addressData.addressName;
		} else {
	...
	methods: {
		...
		pickerConfirm(e) {
			...
			this.addressData.addressName = this.address;
		...
		save() {
			...
			if (!this.$api.helper.isMobile(data.phone)) {
				this.$refs.dzToast.show({
		...
		async handleAddressUpdate(data) {
			await this.$api.http
				.post(
					this.$api.apiConfig.updateAddress,
					{
		...
		async handleAddressCreate(data) {
			await this.$api.http
				.post(this.$api.apiConfig.addAddress, {
					realname: data.realname,
					// mobile_area: data.mobile_area,
					phone: data.phone,
					addressName: data.addressName,
		...
		bindAreaChange() {
			...
			this.addressData.addressDetails = parseResult.detail || '';
			//获取区域
			if (parseResult.area) {
