	...
	<view class="address-box">
		...
		<wwj-scroll-body class="goodlist" @init="mescrollInit" @down="downCallback" @up="upCallback">
			<view v-if="addressList.length > 0">
				<view class="list">
					<view class="row-address" v-for="(row, index) in addressList" :key="index" @tap.stop="select(row)">
						<view class="center">
							<view class="name-tel">
								<view class="name">{{ row.realname }}</view>
								<view class="tel">{{ row.phone }}</view>
								<view class="default" :style="{ color: theme.dzBaseColor, borderColor: theme.dzBaseColor }" v-if="row.isDefault == 1">
									{{ language.address.defaultAddress }}
								</view>
							</view>
							<view class="address">{{ row.addressName }} {{ row.addressDetails }}</view>
						</view>
						<view class="right" @tap.stop="addAddress('edit', row)"><view class="dzicon-iconfont dzicon-post"></view></view>
					</view>
				</view>
			</view>
			<dz-loadmore v-if="loadingType != 'nodata'" :status="loadingType" margin-top="20" margin-bottom="140"></dz-loadmore>
			<view v-if="loadingType == 'nodata'"><dz-empty :text="language.address.noAddress" marginTop="80" :src="emptyAddress"></dz-empty></view>
		</wwj-scroll-body>
		<view class="add">
...
export default {
	...
	methods: {
		...
		async getAddress(page=1) {
			...
			this.loadingType = 'loading';
			try {
				const res = await this.$api.http.get(this.$api.apiConfig.getAddressList, {});
		...
		addAddress(type, address) {
			this.$api.router.push({
				route: this.$api.pagesConfig.addressManage,
				query: {
					type: type,
		...
		async parseWechatAddress(data) {
			...
			if (parseResult.area) {
				...
				let params = {
					...
					addressDetails: data.detailInfo,
					isDefault: 1,
