...
// 弹窗图标url
const $iconUrl = "/static/update/updater.png";
...
function getDownload(data) {
	...
	data.download_url = $mHelper.formatImg(data.download_url);
	
	let dtask;
	let lastProgressValue = 0;
	downloadPopup(popupData, function(res) {
		dtask = plus.downloader.createDownload(data.download_url, {
...
function updatePopup(data, callback) {
	...
	popupView.addEventListener("click", function(e) {
		...
		if(data.is_enforce==1){
			buttonWidth = viewContentWidth
		}
		if (e.clientY > maxTop - 30 && e.clientY < maxTop) {
			if(data.is_enforce==1){
				if (e.clientX > viewContentPadding && e.clientX < maxLeft - viewContentPadding) {
					maskLayer.hide();
					popupView.hide();
					callback && callback();
				}
				...
				if (e.clientX > viewContentPadding && e.clientX < maxLeft - buttonWidth - viewContentPadding) {
					uni.setStorageSync("jumpUpdateVersion", data.version);
					maskLayer.hide();
					popupView.hide();
	...
	maskLayer.addEventListener("click", function() { //处理遮罩层点击
		if(data.mask_click!==false && data.is_enforce!=1){
			maskLayer.hide();
			popupView.hide();
			uni.setStorageSync("jumpUpdateVersion", data.version);
		}
...
export default function(data) {
	updatePopup(data, function() {
		if (/\.wgt$/i.test(data.download_url)) {
			getDownload(data);
		} else {
