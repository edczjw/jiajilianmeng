...
import addressJson from '../../config/areaConfig.js'
...
const provinces = addressJson.reduce((per, cur) => {
    const { child, ...others } = cur
    return per.concat(others)
}, [])

const cities = addressJson.reduce((per, cur) => {
    return per.concat(cur.child.map(({ child, ...others }) => ({ ...others, provinceCode: cur.id })))
}, [])

const areas = addressJson.reduce((per, cur) => {
    const provinceCode = cur.id
    return per.concat(cur.child.reduce((p, c) => {
        const cityCode = c.id
        return p.concat(c.child.map(({ child, ...others }) => ({ ...others, cityCode, provinceCode, })))
    }, []))
}, [])
...
const parseRegionWithRegexp = (fragment, hasParseResult) => {
    ...
    if (province.length === 0) {
        for (let i = 1; i < fragment.length; i++) {
            ...
            const regexProvince = new RegExp(`\{\"id\":\"[0-9]{1,6}\",\"title\":\"${str}[\u4E00-\u9FA5]*?\",\"pid\":\"[0-9]{1,6}\"}`, 'g')
    ...
    if (city.length === 0) {
        for (let i = 1; i < fragment.length; i++) {
            const str = fragment.substring(0, i + 1)
            const regexCity = new RegExp(`\{\"id\":\"[0-9]{1,6}\",\"title\":\"${str}[\u4E00-\u9FA5]*?\",\"pid\":\"[0-9]{1,6}\",\"provinceCode\":\"${province[0] ? `${province[0].id}` : '[0-9]{1,6}'}\"\}`, 'g')
        ...
        if (city[0]) {
            ...
            fragment = fragment.replace(matchStr, '')
            if (province.length === 0) {
                const regexProvince = new RegExp(`\{\"id\":\"${provinceCode}\",\"title\":\"[\u4E00-\u9FA5]+?\",\"pid\":\"[0-9]{1,6}\"}`, 'g')
                const matchProvince = provinceString.match(regexProvince)
    ...
    if (area.length === 0) {
        for (let i = 1; i < fragment.length; i++) {
            const str = fragment.substring(0, i + 1)
            const regexArea = new RegExp(`\{\"id\":\"[0-9]{1,6}\",\"title\":\"${str}[\u4E00-\u9FA5]*?\",\"pid\":\"[0-9]{1,6}\",\"cityCode\":\"${city[0] ? city[0].id : '[0-9]{1,6}'}\",\"provinceCode\":\"${province[0] ? `${province[0].id}` : '[0-9]{1,6}'}\"\}`, 'g')
        ...
        if (area[0]) {
            ...
            if (city.length === 0) {
                const regexCity = new RegExp(`\{\"id\":\"${cityCode}\",\"title\":\"[\u4E00-\u9FA5]+?\",\"pid\":\"[0-9]{1,6}\",\"provinceCode\":\"${provinceCode}\"\}`, 'g')
                const matchCity = cityString.match(regexCity)
...
const parseRegion = (fragment, hasParseResult) => {
    ...
    if (hasParseResult.province[0]) {
        ...
        for (const tempProvince of provinces) {
            const { title } = tempProvince
            let replaceName = ''
            for (let i = title.length; i > 1; i--) {
                const temp = title.substring(0, i)
    ...
    if (hasParseResult.city[0]) {
        ...
        for (const tempCity of cities) {
            const { title, provinceCode } = tempCity
            ...
            if (currentProvince) {
                ...
                // 没有省，市不可能重名
                for (let i = title.length; i > 1; i--) {
                    const replaceName = title.substring(0, i)
                    if (fragment.indexOf(replaceName) === 0) {
                        city.push(tempCity)
                        province.push(provinces.find(item => item.id === provinceCode))
                        fragment = fragment.replace(replaceName, '')
                        break
                    }
                }
    ...
    for (const tempArea of areas) {
        const { title, provinceCode, cityCode } = tempArea
        const currentProvince = province[0]
        const currentCity = city[0]

        // 有省或者市
        if (currentProvince || currentCity) {
            if ((currentProvince && currentProvince.id === provinceCode)
                || (currentCity && currentCity.id) === cityCode) {
                let replaceName = ''
                for (let i = title.length; i > 1; i--) {
                    const temp = title.substring(0, i)
                    if (fragment.indexOf(temp) === 0) {
                ...
                if (replaceName) {
                    area.push(tempArea)
                    !currentCity && city.push(cities.find(item => item.id === cityCode))
                    !currentProvince && province.push(provinces.find(item => item.id === provinceCode))
                    fragment = fragment.replace(replaceName, '')
                    break
                }
            }
            ...
            for (let i = title.length; i > 1; i--) {
                ...
                if (fragment.indexOf(replaceName) === 0) {
                    area.push(tempArea)
                    city.push(cities.find(item => item.id === cityCode))
                    province.push(provinces.find(item => item.id === provinceCode))
                    fragment = fragment.replace(replaceName, '')
                    break
                }
	...
	if(province.length>0 && city.length==0){
		for (const temp of cities) {
		    if(temp.provinceCode===province[0].id){
				city.push(temp);
				break;
			}
		}
	...
	if(city.length>0 && area.length==0){
		for (const temp of areas) {
		    if(temp.cityCode===city[0].id){
				area.push(temp);
				break;
			}
		}
