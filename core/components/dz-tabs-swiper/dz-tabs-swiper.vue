<template>
	<view class="dz-tabs" :style="{
			zIndex: zIndex,
			background: bgColor
		}">
		<scroll-view scroll-x class="dz-scroll-view" :scroll-left="scrollLeft" scroll-with-animation :style="{ zIndex: zIndex + 1 }">
			<view class="dz-tabs-scroll-box" :class="{'dz-tabs-scorll-flex': !isScroll}">
				<view class="dz-tabs-item" :style="[tabItemStyle(index)]"
				 v-for="(item, index) in getTabs" :key="index" :class="[preId + index]" @tap="emit(index)">
					<dz-badge :count="item[count] || item['count'] || 0" :offset="offset" size="mini"></dz-badge>
					{{ item[name] || item['name']}}
				</view>
				<view v-if="showBar" class="dz-scroll-bar" :style="[tabBarStyle]"></view>
			</view>
		</scroll-view>
	</view>
</template>
...
<script>
	 ...
	 * @property {String} name 组件内部读取的list参数中的属性名，（默认name）
	 * @property {String} count 组件内部读取的list参数中的属性名（badge徽标数），同name属性的使用，（默认count）
	 * @property {Array} offset 设置badge徽标数的位置偏移，格式为 [x, y]，也即设置的为top和right的值，单位rpx（默认[5, 20]）
	 * @property {Boolean} bold 激活选项的字体是否加粗（默认true）
	 * @event {Function} change 点击标签时触发
	 * @example <dz-tabs-swiper ref="tabs" :list="list" :is-scroll="false"></dz-tabs-swiper>
	 */
	export default {
		name: "dz-tabs-swiper",
		props: {
		...
		methods: {
			async init() {
				...
				// 颜色渐变过程数组
				this.colorGradientArr = this.$api.helper.colorGradient(this.inactiveColor, this.activeColor, this.colorStep);
			},
			// 获取各个tab的节点信息
			getTabsInfo() {
			...
			getQuery(cb) {
				try {
					let view = uni.createSelectorQuery().in(this).select('.dz-tabs');
					view.fields({
							size: true
						},
						data => {
...
<style scoped lang="scss">
	...
	.dz-tabs-scorll-flex .dz-tabs-item {
		flex: 1;
	}
