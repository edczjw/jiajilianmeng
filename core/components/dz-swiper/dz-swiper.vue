	...
	>
		...
		>
			<swiper-item class="dz-swiper-item" v-for="(item, index) in list" :key="index">
				...
				>
					<image class="dz-swiper-image" :src="item[name] ? $api.helper.formatImg(item[name]) : $api.helper.isObject(item) ? '' : item" :mode="imgMode"></image>
					<view
		...
		>
			<block v-if="mode == 'rect'">
				<view
					...
					:key="index"
					:style="{ background: index == mCurrent ? activeColor : 'rgba(0, 0, 0, 0.3)' }"
				></view>
			</block>
			<block v-if="mode == 'dot'">
				<view
			...
			<block v-if="mode == 'slider'">
				<view  class="dz-indicator-line-box">
					<dz-swiper-line :tabData="list" :activeIndex="mCurrent"
				></dz-swiper-line>
				</view>
			</block>
		</view>
...
export default {
	...
	watch: {
		...
		list(nVal, oVal) {
			if (nVal.length !== oVal.length) this.mCurrent = 0;
		},
	...
	data() {
		return {
			...
			mCurrent: this.current ,// 当前活跃的swiper-item的index
	...
	methods: {
		...
		animationfinish(e) {
			// #ifndef MP-TOUTIAO
			// this.mCurrent = e.detail.current;
			// #endif
		}
	}
};
