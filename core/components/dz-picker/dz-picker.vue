<template>
	<dz-popup :maskCloseAble="maskCloseAble" mode="bottom" :borderRadius="borderRadius" :popup="false" v-model="value" length="auto" :safeAreaInsetBottom="safeAreaInsetBottom" @close="close" :z-index="mZIndex">
		<view class="dz-datetime-picker">
			<view class="dz-picker-header" @touchmove.stop.prevent="">
				<view class="dz-btn-picker dz-btn-picker--tips" 
			...
			<view class="dz-picker-body">
				...
				<picker-view v-else-if="mode == 'multiSelector'" :value="valueArr" @change="change" class="dz-picker-view" @pickstart="pickstart" @pickend="pickend">
...
export default {
	...
	watch: {
		...
		provinceChange(val){
			...
			this.valueArr[1]=0;
			this.valueArr[2]=0;
			this.citys = areaConfig[this.province].child;
			this.areas = areaConfig[this.province].child[this.city].child;
		},
		cityChange(val){
			this.area = 0;
			this.valueArr[2]=0;
	...
	methods: {
		...
		setCitys() {
			...
			areaConfig[this.province].child.map((v, k) => {
				if (useCode ? v.id == tmp : v.title == tmp) {
					tmp = k;
				}
			});
		...
		setAreas() {
			...
			areaConfig[this.province].child[this.city].child.map((v, k) => {
				if (useCode ? v.id == tmp : v.title == tmp) {
					tmp = k;
				}
			});
		...
		getResult(event = null) {
			...
			if (this.mode == 'time') {
				...
				if (this.params.province) result.province = areaConfig[this.province];
				if (this.params.city) result.city = areaConfig[this.province].child[this.city];
				if (this.params.area) result.area = areaConfig[this.province].child[this.city].child[this.area];
			} else if (this.mode == 'selector') {
