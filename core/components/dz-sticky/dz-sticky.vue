	...
	export default {
			...
			style() {
				...
				if(!this.disabled) {
					if (this.cssSticky) {
						...
						style.top = this.$api.components.addUnit(this.stickyTop,'px')
				...
				return this.$api.helper.deepMerge(this.$api.helper.addStyle(this.customStyle), style)
			},
			// 吸顶内容的样式
			stickyContent() {
				const style = {}
		...
		methods: {
			...
			initObserveContent() {
				...
				this.$mGetRect('#' + this.elId).then((res) => {
			...
			async checkSupportCssSticky() {
				...
				if (this.$api.systemInfo.platform === 'android' && Number(this.$api.systemInfo.sys().system) > 8) {
					this.cssSticky = true
				}
