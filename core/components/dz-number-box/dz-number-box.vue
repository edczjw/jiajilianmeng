<template>
	<view class="dz-numberbox">
		<view class="dz-icon-minus" @touchstart.stop.prevent="btnTouchStart('minus')" @touchend.stop.prevent="clearTimer" :class="{ 'dz-icon-disabled': disabled || inputVal <= min }"
		    :style="{
				background: bgColor,
				height: inputHeight + 'rpx',
				color: color
			}">
			<dz-icon name="move" :size="size"></dz-icon>
		</view>
		<input :disabled="disabledInput || disabled" :cursor-spacing="getCursorSpacing" :class="{ 'dz-input-disabled': disabled }"
		    v-model="inputVal" class="dz-number-input" @blur="onBlur"  @focus="onFocus"
		    type="number" :style="{
				color: color,
				fontSize: size + 'rpx',
				background: bgColor,
		...
		<view class="dz-icon-plus" @touchstart.stop.prevent="btnTouchStart('plus')" @touchend.stop.prevent="clearTimer" :class="{ 'dz-icon-disabled': disabled || inputVal >= max }"
		    :style="{
				background: bgColor,
				height: inputHeight + 'rpx',
				color: color
			}">
			<dz-icon name="add1" :size="size"></dz-icon>
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @example <dz-number-box :min="1" :max="100"></dz-number-box>
	 */
	export default {
		name: "dz-number-box",
		props: {
		...
		watch: {
			...
			inputVal(v1, v2) {
				...
				let tmp = this.$api.helper.isNumber(v1);
