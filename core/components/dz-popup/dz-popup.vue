<template>
	<view v-if="visibleSync" :style="[customStyle, {
		zIndex: mZindex - 1
	}]" :class="{ 'dz-drawer-visible': showDrawer }" class="dz-drawer" hover-stop-propagation>
		<dz-mask :duration="duration" :custom-style="maskCustomStyle" :maskClickAble="maskCloseAble" :z-index="mZindex - 2" :show="showDrawer && mask" @click="maskClick"></dz-mask>
		<view
			...
			:class="[
				...
				'dz-drawer-' + mode,
				showDrawer ? 'dz-drawer-content-visible' : '',
		...
		>
			...
			<view @tap="close" class="dz-close" :class="['dz-close--' + closeIconPos]">
				<dz-icon
...
export default {
	...
	computed: {
		...
		centerStyle() {
			...
			style.zIndex = this.mZindex;
			style.marginTop = `-${this.$api.components.addUnit(this.negativeTop)}`;
			style.transform = `scale(${this.showDrawer ? 1 : 0})`
