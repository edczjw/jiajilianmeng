<template>
	<view class="dz-field" :class="{'dz-border-top': borderTop, 'dz-border-bottom': borderBottom }">
		<view class="dz-field-inner" :class="[type == 'textarea' ? 'dz-textarea-inner' : '', 'dz-label-postion-' + labelPosition]">
			<view class="dz-label" :class="[required ? 'dz-required' : '']" :style="{
				...
				<slot name="icon"></slot>
				<text class="dz-label-text" :class="[this.$slots.icon || icon ? 'dz-label-left-gap' : '']">{{ label }}</text>
			</view>
			<view class="fild-body">
				<view class="dz-flex-1 dz-flex" :style="[inputWrapStyle]">
					<textarea v-if="type == 'textarea'" class="dz-flex-1 dz-textarea-class" :style="[fieldStyle]" :value="value"
				...
				<dz-icon :size="clearSize" v-if="clearable && value != '' && focused" name="roundclosefill" color="#c0c4cc" class="dz-clear-icon" @click="onClear"/>
				<view class="dz-button-wrap"><slot name="right" /></view>
				<dz-icon v-if="rightIcon" @click="rightIconClick" :name="rightIcon" color="#c0c4cc" :style="[rightIconStyle]" size="26" class="dz-arror-right" />
			</view>
		</view>
		<view v-if="errorMessage !== false && errorMessage != ''" class="dz-error-message" :style="{
