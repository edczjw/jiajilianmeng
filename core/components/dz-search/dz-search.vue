<template>
	<view class="dz-search" @tap="clickHandler" :style="{
		margin: margin,
	}">
		<view
			class="dz-content"
			:style="{
				...
				borderRadius: shape == 'round' ? '100rpx' : '15rpx',
				border: borderStyle,
				height: $api.components.addUnit(height)
			}"
		>
			<view class="dz-icon-wrap">
				<dz-icon class="dz-clear-icon" :size="30" :name="searchIcon" :color="searchIconColor ? searchIconColor : color"></dz-icon>
			</view>
			<input
				confirm-type="search"
				@blur="blur"
				...
				:maxlength="maxlength"
				:focus="focus"
				placeholder-class="dz-placeholder-class"
			...
			/>
			<view class="dz-close-wrap" v-if="keyword && clearabled && focused" @tap="clear">
				<dz-icon class="dz-clear-icon" name="roundclosefill" size="34" color="#c0c4cc"></dz-icon>
			</view>
		</view>
		<view :style="[actionStyle]" class="dz-action" 
			:class="[showActionBtn || show ? 'dz-action-active' : '']" 
 ...
 * @property {String} search-icon 输入框左边的图标，可以为图标名称或图片路径
 ...
 * @property {String | Number} height 输入框高度，单位rpx,px（默认64）
 * @event {Function} change 输入框内容发生变化时触发
 * @event {Function} search 用户确定搜索时触发，用户按回车键，或者手机键盘右下角的"搜索"键时触发
 * @event {Function} custom 用户点击右侧控件时触发
 * @event {Function} clear 用户点击清除按钮时触发
 * @example <dz-search placeholder="日照香炉生紫烟" v-model="keyword"></dz-search>
 */
export default {
	name: "dz-search",
	props: {
