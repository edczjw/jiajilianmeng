<template>
	<view class="dz-avatar" :style="[wrapStyle]" @tap="click">
		<image
			@error="loadError"
			:style="[imgStyle]"
			class="dz-avatar__img"
			v-if="!mText && avatar"
			:src="avatar" 
			:mode="imgMode"
		></image>
		<text class="dz-line-1" v-else-if="mText" :style="{
			fontSize: '38rpx'
		}">{{mText}}</text>
		<slot v-else></slot>
		<view class="dz-avatar__sex" v-if="showSex" :class="['dz-avatar__sex--' + sexIcon]" :style="[mSexStyle]">
			<dz-icon :name="sexIcon" size="20"></dz-icon>
		</view>
		<view class="dz-avatar__level" v-if="showLevel" :style="[mLevelStyle]">
			<dz-icon :name="levelIcon" size="20"></dz-icon>
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @example <dz-avatar :src="src"></dz-avatar>
	 */
	export default {
		name: 'dz-avatar',
		props: {
		...
		computed: {
			wrapStyle() {
				...
				style.border =  this.isBorder ? `${this.borderSize}rpx solid ${this.borderColor}` : '';
