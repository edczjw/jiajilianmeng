<template>
	<view class="dz-select">
		<!-- <view class="dz-select__action" :class="{
			'dz-select--border': border
		}" @tap.stop="selectHandler">
			<view class="dz-select__action__icon" :class="{
				'dz-select__action__icon--reverse': value == true
			}">
				<dz-icon name="arrow-down-fill" size="26" color="#c0c4cc"></dz-icon>
			</view>
		</view> -->
		<dz-popup :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" :borderRadius="borderRadius" v-model="value" length="auto" :safeAreaInsetBottom="safeAreaInsetBottom" @close="close" :z-index="mZIndex">
			<view class="dz-select">
...
export default {
	...
	methods: {
		...
		setDefaultSelector() {
			...
			this.lastSelectIndex = this.$api.helper.deepClone(this.defaultSelector);
		},
		// 计算列数
		setColumnNum() {
			// 单列的列数为1
