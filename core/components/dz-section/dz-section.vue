<template>
	<view class="dz-section">
		<view class="dz-section__title" :style="{
			...
			<view class="dz-section__title__icon-wrap dz-flex" :style="[lineStyle]" v-if="showLine">
				<dz-icon top="0" name="column-line" :size="fontSize * 1.25" bold :color="lineColor ? lineColor : color"></dz-icon>
			</view>
		
			<text class="dz-flex dz-section__title__text">{{title}}</text>
		</view>
		<view class="dz-section__right-info" v-if="right || $slots.right" :style="{
			color: subColor
		}" @tap="rightClick"> 
			<slot name="right" v-if="$slots.right" />
			<block v-else>
				{{subTitle}}
				<view class="dz-section__right-info__icon-arrow dz-flex" v-if="arrow">
		...
		computed: {
			...
			lineStyle() {
				...
				return {
					...
					top: -(Number(this.fontSize) * (this.$api.systemInfo.platform == 'ios' ? 0.14 : 0.15)) + 'rpx',
				}
			}
		},
		methods: {
