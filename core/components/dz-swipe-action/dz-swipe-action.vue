<template>
	<view class="">
		<movable-area class="dz-swipe-action" :style="{ backgroundColor: bgColor }">
			<movable-view
				class="dz-swipe-view"
				@change="change"
				@touchend="touchend"
			...
			>
				<view
					class="dz-swipe-content"
					@tap.stop="contentClick"
				>
					<slot></slot>
				</view>
				<view class="dz-swipe-del" v-if="showBtn" @tap.stop="btnClick(index)" :style="[btnStyle(item.style)]" v-for="(item, index) in options" :key="index">
					<view class="dz-btn-text">{{ item.text }}</view>
				</view>
			</movable-view>
		</movable-area>
	</view>
 ...
 * @example <dz-swipe-action btn-text="收藏">...</dz-swipe-action>
 */
export default {
	name: 'dz-swipe-action',
	props: {
	...
	data() {
		return {
			...
			elId: this.$api.helper.guid(), // id，用于通知另外组件关闭时的识别
	...
	methods: {
		...
		touchstart() {},
		getActionRect() {
			this.$mGetRect('.dz-swipe-action').then(res => {
