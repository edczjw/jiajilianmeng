<template>
	<view class="">
		...
		<!-- 解决fixed定位后导航栏塌陷的问题 -->
		<view class="dz-navbar-placeholder" v-if="isFixed && !immersive" :style="{ width: '100%', height: Number(navbarHeight) + statusBarHeight + 'px' }"></view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			navbarStyle() {
				let style = {};
				style.zIndex = this.zIndex ? this.zIndex : this.$api.zIndex.navbar;
				// 合并用户传递的背景色对象
		...
		methods: {
			goBack() {
				...
				if (typeof this.customBack === 'function') {
					...
					this.customBack.bind(this.$api.components.$parent.call(this))();
				} else {
					// 获取当前页面栈，如果页面栈为1，则直接跳转到首页
					let pages = getCurrentPages();
					let indexPage = this.backRelaunchUrl ? this.backRelaunchUrl : this.$api.appConfig.indexPath;
					if(pages.length==1){
						uni.reLaunch({
							url:indexPage
						})
