<template>
	<view
		...
		:class="{ 'dz-bar-line': opacity > 0.85 && splitLine, 'dz-navbar-fixed': isFixed, 'dz-backdrop__filter': backdropFilter }"
	...
	>
		...
		<view class="dz-navigation_bar-title" :style="{ opacity: opacity, color: color, paddingTop: top - statusBarHeight + 'px' }" v-if="title && !isCustom">{{ title }}</view>
		<slot />
	</view>
</template>
			...
			if (this.dropDownHide) {
				if (this.scrollTop < 0) {
					if (this.dropDownOpacity > 0) {
						this.dropDownOpacity = 1 - Math.abs(this.scrollTop) / 20;
					}
				} else {
