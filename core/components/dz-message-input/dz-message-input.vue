<template>
	<view class="dz-char-box">
		<view class="dz-char-flex">
			<input :disabled="disabledKeyboard" :value="valueModel" type="number" :focus="focus" :maxlength="maxlength" class="dz-input" @input="getVal"/>
			<view v-for="(item, index) in loopCharArr" :key="index">
				<view :class="[breathe && charArrLength == index ? 'dz-breathe' : '', 'dz-char-item',
				charArrLength === index && mode == 'box' ? 'dz-box-active' : '',
				mode === 'box' ? 'dz-box' : '']" :style="{
					...
					<view class="dz-placeholder-line" :style="{
							display: charArrLength === index ? 'block' : 'none',
							height: width * 0.5 +'rpx'
						}"
						v-if="mode !== 'middleLine'"
					></view>
					<view v-if="mode === 'middleLine' && charArrLength <= index" :class="[breathe && charArrLength == index ? 'dz-breathe' : '', charArrLength === index ? 'dz-middle-line-active' : '']"
					 class="dz-middle-line" :style="{height: bold ? '4px' : '2px', background: charArrLength === index ? activeColor : inactiveColor}"></view>
					<view v-if="mode === 'bottomLine'" :class="[breathe && charArrLength == index ? 'dz-breathe' : '', charArrLength === index ? 'dz-buttom-line-active' : '']"
					 class="dz-bottom-line" :style="{height: bold ? '4px' : '2px', background: charArrLength === index ? activeColor : inactiveColor}"></view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			animationClass() {
				return (index) => {
					if (this.breathe && this.charArr.length == index) return 'dz-breathe';
					else return '';
				}
			},
			// 用于显示字符
