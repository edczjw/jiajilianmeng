<template>
	<view class="dz-image" @tap="onClick" :style="[wrapStyle, backgroundStyle]">
		<image
			...
			:style="{
				borderRadius: shape == 'circle' ? '50%' : $api.components.addUnit(borderRadius)
			}"
		></image>
		<view
			v-if="showLoading && loading"
			class="dz-image__loading"
			:style="{
				borderRadius: shape == 'circle' ? '50%' : $api.components.addUnit(borderRadius),
 ...
 * @property {Boolean} show-mendz-by-longpress 是否开启长按图片显示识别小程序码菜单，仅微信小程序有效（默认 false）
 ...
 * @example <dz-image width="100%" height="300rpx" :src="src"></dz-image>
 */
export default {
	name: 'dz-image',
	props: {
	...
	computed: {
		wrapStyle() {
			...
			style.width = this.$api.components.addUnit(this.width);
			style.height = this.$api.components.addUnit(this.height);
			// 如果是配置了圆形，设置50%的圆角，否则按照默认的配置值
			style.borderRadius = this.shape == 'circle' ? '50%' : this.$api.components.addUnit(this.borderRadius);
