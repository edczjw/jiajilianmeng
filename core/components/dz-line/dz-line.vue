<template>
	<view class="dz-line" :style="[lineStyle]">
...
<script>
	 ...
	 * @example <dz-line color="red"></dz-line>
	 */
	export default {
		name: 'dz-line',
		props: {
		...
		computed: {
			lineStyle() {
				...
				if(this.direction == 'row') {
					...
					style.width = this.$api.components.addUnit(this.length);
					if(this.hairLine) style.transform = 'scaleY(0.5)';
				} else {
					// 如果是竖向线条，边框宽度为1px，再通过transform缩小一半，就是0.5px了
					style.borderLeftWidth = '1px';
					style.borderLeftStyle = this.borderStyle;
					style.height = this.$api.components.addUnit(this.length);
